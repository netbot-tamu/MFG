...
struct Data_BA_PTS_REL
{
	...
	Data_BA_PTS_REL(vector<KeyPoint3d>& kp_, vector<View>& views_) : kp(kp_), views(views_) {}

};

void costFun_BA_Pts_Rel(double *p, double *error, int numPara, int numMeas, void *adata)
{
	struct Data_BA_PTS_REL* dp = (struct Data_BA_PTS_REL *) adata;
	double kernelPt = 2;
	double kernelLn = 5;
	double kernelVpLn = 5;
...
void Mfg::adjustBundle_Pt_Rel (int numPos, int numFrm)
...
//	int numFrm = 5;	// number of frames to provide measurements (for reprojection error)
	...
	Data_BA_PTS_REL data(keyPoints, views);
	...
	int ret = dlevmar_dif(costFun_BA_Pts_Rel, para, meas, numPara, numMeas,
