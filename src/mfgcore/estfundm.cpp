...
/********************************************************************************
 * Estimate fundamental or essential matrices
 ********************************************************************************/

// Methods of estimating fundamental matrix / essential matrix
// using nister's 5-point algorithm
#include "consts.h"
#include "utils.h"
...

{
    ...
    double threshDist = 1;		//  in image units 0.64
...
// q1 and q2 must be normalized points in image coordinates
{
    ...
    R1.copyTo(Pa.colRange(0, 3));
    t.copyTo(Pa.col(3));
    R1.copyTo(Pb.colRange(0, 3));
    t2.copyTo(Pb.col(3));
    R2.copyTo(Pc.colRange(0, 3));
    t.copyTo(Pc.col(3));
    R2.copyTo(Pd.colRange(0, 3));
    t2.copyTo(Pd.col(3));
