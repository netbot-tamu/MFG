...
// ----- G2O parameter setting -----
	...
	//	((g2o::SBACam*)v_cam)->setKcam(K.at<double>(0,0),K.at<double>(1,1),K.at<double>(0,2),K.at<double>(1,2),0);//???
  	...
  	//-------- camera to camera distance constraints ---------
	vector<g2o::EdgeCamCamDist*> edges_camdist;
...
cout<<"LBA fixed points number "<<kptIdx2Rpj_notOpt.size()<<", non-fixed "<<kptIdx2Opt.size()<<endl;
	...
	for(int i=0; i<edges_camdist.size(); ++i) {
		edges_camdist[i]->computeError();
	//	cout<<"hard-constraint error = "<<edges_camdist[i]->chi2()<<endl;
	}
